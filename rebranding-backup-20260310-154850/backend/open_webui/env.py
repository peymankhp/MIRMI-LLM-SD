...
# OPEN_WEBUI_DIR should be the directory where env.py resides (mirmi_llm/)
OPEN_WEBUI_DIR = ENV_FILE_PATH.parent

# BACKEND_DIR is the parent of OPEN_WEBUI_DIR (backend/)
BACKEND_DIR = OPEN_WEBUI_DIR.parent

# BASE_DIR is the parent of BACKEND_DIR (mirmi-llm-dev/)
...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "MIRMI LLM")
if WEBUI_NAME != "MIRMI LLM":
    WEBUI_NAME += " (MIRMI LLM)"
...
if FROM_INIT_PY:
    PACKAGE_DATA = {"version": importlib.metadata.version("mirmi-llm")}
else:
    try:
...
try:
    ...
    changelog_content = (pkgutil.get_data("mirmi_llm", "CHANGELOG.md") or b"").decode()
...
FORWARD_USER_INFO_HEADER_USER_NAME = os.environ.get(
    "FORWARD_USER_INFO_HEADER_USER_NAME", "X-MIRMILLM-User-Name"
)
FORWARD_USER_INFO_HEADER_USER_ID = os.environ.get(
    "FORWARD_USER_INFO_HEADER_USER_ID", "X-MIRMILLM-User-Id"
)
FORWARD_USER_INFO_HEADER_USER_EMAIL = os.environ.get(
    "FORWARD_USER_INFO_HEADER_USER_EMAIL", "X-MIRMILLM-User-Email"
)
FORWARD_USER_INFO_HEADER_USER_ROLE = os.environ.get(
    "FORWARD_USER_INFO_HEADER_USER_ROLE", "X-MIRMILLM-User-Role"
)
...
FORWARD_SESSION_INFO_HEADER_MESSAGE_ID = os.environ.get(
    "FORWARD_SESSION_INFO_HEADER_MESSAGE_ID", "X-MIRMILLM-Message-Id"
)
FORWARD_SESSION_INFO_HEADER_CHAT_ID = os.environ.get(
    "FORWARD_SESSION_INFO_HEADER_CHAT_ID", "X-MIRMILLM-Chat-Id"
)
...
if FROM_INIT_PY:
    ...
    if DATA_DIR.exists() and DATA_DIR != NEW_DATA_DIR:
        ...
        shutil.make_archive(DATA_DIR.parent / "mirmi_llm_data", "zip", DATA_DIR)
...
REDIS_KEY_PREFIX = os.environ.get("REDIS_KEY_PREFIX", "mirmi-llm")
...
# Allows external apps to exchange OAuth tokens for MIRMILLM tokens
...
OTEL_SERVICE_NAME = os.environ.get("OTEL_SERVICE_NAME", "mirmi-llm")
