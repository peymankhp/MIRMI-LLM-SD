...
# This script replaces all "Open WebUI" references with "MIRMI LLM"
...
# 1. Replace "Open WebUI" with "MIRMI LLM"
echo ""
echo "Step 1: Replacing 'Open WebUI' with 'MIRMI LLM'..."
replace_in_files "Open WebUI" "MIRMI LLM" "'Open WebUI' → 'MIRMI LLM'"

# 2. Replace "open-webui" with "mirmi-llm"
echo ""
echo "Step 2: Replacing 'open-webui' with 'mirmi-llm'..."
replace_in_files "open-webui" "mirmi-llm" "'open-webui' → 'mirmi-llm'"

# 3. Replace "OpenWebUI" with "MIRMILLM"
echo ""
echo "Step 3: Replacing 'OpenWebUI' with 'MIRMILLM'..."
replace_in_files "OpenWebUI" "MIRMILLM" "'OpenWebUI' → 'MIRMILLM'"
...
replace_in_files "https://github\.com/open-webui/open-webui[^\"' ]*" "https://github.com/peymankhp/MIRMI-LLM" "GitHub URLs"
replace_in_files "https://github\.com/open-webui/docs[^\"' ]*" "" "GitHub docs URLs"
replace_in_files "https://github\.com/open-webui/pipelines[^\"' ]*" "" "GitHub pipelines URLs"

# Remove Discord links
replace_in_files "https://discord\.gg/5rJgQTnV4s" "" "Discord invite links"

# Remove social media references
replace_in_files "https://twitter\.com/OpenWebUI" "" "Twitter/X links"
replace_in_files "https://www\.reddit\.com/r/OpenWebUI" "" "Reddit links"
...
if [ -f "package.json" ]; then
    ...
    sed -i 's/"name": "open-webui"/"name": "mirmi-llm"/g' package.json
    sed -i 's/"Open WebUI"/"MIRMI LLM"/g' package.json
fi
...
for compose_file in docker-compose*.yaml docker-compose*.yml; do
    if [ -f "$compose_file" ]; then
        ...
        sed -i 's/container_name: open-webui/container_name: mirmi-llm/g' "$compose_file"
        sed -i 's/image: ghcr\.io\/open-webui\/open-webui/image: ghcr.io\/peymankhp\/mirmi-llm/g' "$compose_file"
    fi
done
...
find src -type f -name "*.svelte" 2>/dev/null | while read -r file; do
    ...
    sed -i '/href="https:\/\/twitter\.com\/OpenWebUI/d' "$file"
done
...
echo ""
echo "📊 Summary:"
echo "   - Replaced 'Open WebUI' with 'MIRMI LLM'"
echo "   - Replaced 'open-webui' with 'mirmi-llm'"
